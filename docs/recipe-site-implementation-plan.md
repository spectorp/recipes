...
- **Polish** — README documents add/edit recipe, units, category keys; larger touch targets on browse/filters/theme/scale; empty catalog state when zero recipes. (Phase 11)
- **Browse filters** — client-side only; filter/search state and Filters panel open/closed live in a React context so they **persist** when opening a recipe and navigating back (not URL query params yet). **Search** sits beside the page title; facets (including **min rating** as a checkbox column) live in a **collapsed-by-default** Filters panel. Search is **token AND**; with a query, **title/id hits rank above** other-field hits, then title A–Z. Facet checkboxes are **contextual**. Facet **AND/OR** toggle applies across category columns + tags + rating (within a column stays any-of); search stays a hard AND. Default `facetMode: 'and'`. No sort or source dropdowns. (Phase 5+)
...
11. **Polish** — ✅ empty states, mobile touch targets, README (edit via JSON,
    units, category keys, local/deploy)
